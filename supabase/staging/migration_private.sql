-- DESIGN ONLY. Do not apply this file as a Supabase migration without review.
-- This schema contains migration control data and sanitized legacy records. It
-- must remain outside the Data API's exposed schemas.

-- Fail closed if the schema already exists; its ownership and contents must be
-- inspected before any design is applied. The no-login owner must be created
-- by an administrator, must not be granted to the application runtime role,
-- and must exist before this design is converted into a reviewed migration.
create schema migration_private authorization migration_private_owner;
set role migration_private_owner;

revoke all on schema migration_private from public, anon, authenticated, service_role, postgres;
alter default privileges in schema migration_private revoke all on tables from public, anon, authenticated, service_role, postgres;
alter default privileges in schema migration_private revoke all on sequences from public, anon, authenticated, service_role, postgres;
alter default privileges in schema migration_private revoke all on functions from public, anon, authenticated, service_role, postgres;
...
create table migration_private.extraction_summary (
  ...
  check (
    coalesce(eligible_count, 0)
      + coalesce(excluded_count, 0)
      + coalesce(quarantined_count, 0)
      <= extracted_count
  )
);
...
revoke all on all tables in schema migration_private from public, anon, authenticated, service_role, postgres;
revoke all on all sequences in schema migration_private from public, anon, authenticated, service_role, postgres;
revoke all on all functions in schema migration_private from public, anon, authenticated, service_role, postgres;
...
comment on table migration_private.legacy_membership_archive is
  'Must never contain password hashes, login tokens, OAuth secrets, raw Meteor services data, or legacy task/rating history.';
